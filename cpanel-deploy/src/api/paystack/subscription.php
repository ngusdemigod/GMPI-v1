 ...
 * 
 * Creates a recurring subscription payment for weekly, monthly, or annual donations
 */
...
$required = ['email', 'amount', 'currency', 'frequency'];
...
// Validate frequency — must match values accepted by PaystackService
$validFrequencies = ['weekly', 'monthly', 'annually'];
if (!in_array($frequency, $validFrequencies)) {
    respondJson(422, [
        'success' => false,
        'error' => 'Invalid frequency. Must be: weekly, monthly, or annually'
    ]);
}
...
$category = $input['category'] ?? 'Recurring Donation';
$description = $input['description'] ?? 'Recurring donation to Bright Light Ministry Int\'l';
...
try {
    ...
    if (isset($response['success']) && $response['success'] === true && isset($response['data'])) {
        ...
        error_log("Subscription response: " . json_encode($response));
        echo json_encode([
            'success' => false,
    ...
    error_log("Paystack subscription error: " . $e->getMessage());
    echo json_encode([
        'success' => false,
